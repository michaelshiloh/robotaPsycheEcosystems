...
class SarahsEcosystem {  
  ...
  class Dictator {
    ...
    void checkEdges() {
      if (location.x > zoneWidth - shapeWidth) {
        velocity.x *= -1;
      } else if (location.x < 400 + shapeWidth) {
        velocity.x *= -1;
      }

      if (location.y > zoneHeight - shapeWidth) {
        velocity.y *= -1;
  ...
  void setup() {
    ...
    numberOfRevs = 15;
    numberOfElites = 4; 
    dict = new Dictator(zoneWidth/2, zoneHeight/2); 
    shelter = new safeZone(); 

    for (int i = 0; i < numberOfRevs; i++) {
      revs.add(new Rev(random(0.1, 2.5), random(zoneWidth), random(zoneHeight))); // initial location
    }

    for (int i = 0; i < numberOfElites; i++) {
      elites.add(new Elite(random(400+30, zoneWidth), random(zoneHeight))); // initial location
    }
  }
